...
// load posts once DOM has loaded
...
function deletePost(e) {
  // event propogation - also list items contain 
  // all list items contains .delete class, so we need to target just the one we click
  if (e.target.parentElement.classList.contains('delete')) {
    // dataset = data- attribute in HTML.
    // id is the right hand side of our data-id
    // data-id is accedd by dataset.id
    var id = e.target.parentElement.dataset.id;
    if (confirm('Are you sure?')) {
      _http.http.delete('http://localhost:3000/posts/' + id).then(function (data) {
        _ui.ui.showAlert('Item Remove', 'alert alert-success');
        getPosts();
      }).catch(function (err) {
        return console.log(err);
      });
